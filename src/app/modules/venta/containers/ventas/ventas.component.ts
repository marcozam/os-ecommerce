...
export class VentasComponent implements OnInit {
  ...
  @ViewChild('ticketVenta', { static: true }) ticketVenta: ViewContainerRef;
          ...
          .subscribe((inv: Inventario) => {
            if (inv.cantidad <= 0) {
              this.dialog.openDialog(
                ...
                {
                  showButtons: true,
                  onClose: res => { console.log(res ? 'No hacer nada' : 'Remover producto'); }
                });
            }
          });
      });
      ...
      if (pagos) {
        ...
        this._ventaService.saveVenta(this.venta, this.sucursalID, (newVenta: Venta) => {
          if (newVenta) {
            ...
            this.dialog.openDialog('Error', 'Ocurrio un error al generar la venta');
