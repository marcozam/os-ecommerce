...
export class BaseAjaxService {
    ...
    getData(data: any): Observable<any> {
        if (!this.online || environment.production) {
            this.openDialog(ERROR_TITLE, 'No hay conexion');
        }
                ...
                map((result: AjaxRequestResult) => {
                    switch (result.code) {
                        case 'Success':
                            return result.data;
                        case 'AuthError':
                            this.openDialog(WARNING_TITLE, AUTH_ERROR_MESSAGE);
                            throwError(AUTH_ERROR_MESSAGE);
                            break;
                        // General Error
                        default:
                            this.openSnackBar(INTERNAL_SERVER_ERROR_MESSAGE);
